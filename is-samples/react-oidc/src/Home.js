...
import ViewMessages from './viewmesseges';
...
class HomeComponent extends React.Component {
    state = {
     ...
     componentWillMount() {
        ...
        if (isValidSession()) {
            ...
            console.log("decodeIdToken(session.ID_TOKEN)",decodeIdToken(session.ID_TOKEN).groups)
            this.setState({
                user_role:decodeIdToken(session.ID_TOKEN).groups,
    ...
    render() {
        const { tokenResponse, idToken, isLoggedIn , user_role } = this.state;
        return (
